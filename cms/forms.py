...
class BannerBlockForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk and self.instance.payload:
            ...
            self.fields['button_text'].initial = self.instance.payload.get('button_text', '')
            self.fields['button_link'].initial = self.instance.payload.get('button_link', '')
    ...
    def save(self, commit=True):
        ...
        if image:
            ...
            payload['image_url'] = self.instance.payload.get('image_url', '') if self.instance.pk else ''
...
class CategoryGridBlockForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance.pk and self.instance.payload:
            self.fields['title'].initial = self.instance.payload.get('title', 'Наше меню')
    ...
    def save(self, commit=True):
        ...
        category_ids = [
            int(x.strip()) for x in category_ids_str.split(',') 
            if x.strip().isdigit()
        ]
...
class ProductCarouselBlockForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance.pk and self.instance.payload:
            self.fields['title'].initial = self.instance.payload.get('title', 'Хиты продаж')
    ...
    def save(self, commit=True):
        ...
        product_slugs = [
            x.strip() for x in product_slugs_str.split(',') 
            if x.strip()
        ]
